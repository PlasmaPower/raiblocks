...
int mdb_cmp (MDB_txn *, MDB_dbi, MDB_val const * val_a, MDB_val const * val_b)
{
	auto result (std::memcmp (val_a->mv_data, val_b->mv_data, std::min (val_a->mv_size, val_b->mv_size)));
	if (result == 0)
	{
		if (val_a->mv_size < val_b->mv_size)
		{
			result = -1;
		}
		else if (val_a->mv_size > val_b->mv_size)
		{
			result = 1;
		}
	}
