...
check_bootstrap_weights (true)
{
	if (rai::rai_network == rai::rai_networks::rai_live_network)
	{
		...
		const char * bootstrap_weights_str = rai::bootstrap_weights;
		while (*bootstrap_weights_str)
		{
			const char * next_line = strchr (bootstrap_weights_str, '\n');
			if (!next_line)
			{
				next_line = strchr (bootstrap_weights_str, '\0');
			}
			if (*bootstrap_weights_str != '#')
			{
				const char * next_space = strchr (bootstrap_weights_str, ' ');
				if (next_space < next_line)
				{
					rai::account rep;
					if (!rep.decode_account (std::string (bootstrap_weights_str, next_space - bootstrap_weights_str)))
					{
						bootstrap_weights_str = next_space + 1;
						rai::amount stake;
						if (!stake.decode_dec (std::string (bootstrap_weights_str, next_line - bootstrap_weights_str)))
						{
							bootstrap_weights[rep] = stake.number();
						}
					}
				}
			}
			bootstrap_weights_str = next_line;
		}
	}
}
